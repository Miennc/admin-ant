...
const validate = () => {
  ...
  error.value.oldPass = oldPass.value === newPass.value ? '' : 'Hai mật khẩu không khớp nhau'
...
<template>
  <div class=" px-3  py-3">
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
      <div class="px-5 py-2">
        <div class="text-lg text-[#111827] font-[500]">
          Thông tin cá nhân
        </div>
        ...
        <div>
          <a-input-password v-model:value="newPass" placeholder="mật khẩu mới"/>
          <span class="text-red-500 italic text-sm">{{ error.newPass }} </span>
        </div>
        <div class="mt-2">
          <a-input-password v-model:value="oldPass" placeholder=" mật lại mật khẩu mới"/>
          <span class="text-red-500 italic text-sm"> {{ error.oldPass }} </span>
        </div>

        <div class="mt-2">
          <a-button type="primary" @click="changePass">Thay đổi mật khẩu</a-button>
        </div>
