...
const validate = () => {
  errors.value.name = inputLevel.value.name
    ? ""
    : "Tên danh level không được để trống";
  errors.value.fileUpload =
    inputLevel?.value?.fileUpload?.length < 1 ? "Ảnh không được để trống" : "";
  errors.value.valueNumber =
    regexValue.test(inputLevel.value.valueNumber) &&
    inputLevel.value.valueNumber
      ? ""
      : "Giá trị không hợp lệ";
  return (
    !errors.value.name && !errors.value.fileUpload && !errors.value.valueNumber
  );
};
...
<template>
    ...
    <div>
      ...
      >
        ...
        <div class="mt-2">
          ...
          <div class="mt-1">
            <a-input
              v-model:value="inputLevel.valueNumber"
              placeholder="Giá trị"
            />
          </div>
          <span class="text-red-500 font-medium italic text-sm">
            {{ errors.valueNumber }}</span
          >
        </div>
        ...
        <div
          ...
          v-if="inputLevel.fileUrl"
        >
          <img
            class="w-full h-full object-cover"
            :src="inputLevel.fileUrl"
            alt=""
          />
        ...
        <div class="py-2 inline-block min-w-full">
          <div class="overflow-hidden">
            <table class="min-w-full">
              ...
              <tbody
                v-for="(itemLevel, indexLevel) in dataLevel"
                :key="indexLevel"
              >
                <tr
  ...
  >
    <div>
      <label class="block text-sm font-medium text-gray-700"> Tên level </label>
      <div class="mt-1">
        <a-input v-model:value="inputLevel.name" placeholder="Tên level" />
      </div>
      <span class="text-red-500 font-medium italic text-sm">
